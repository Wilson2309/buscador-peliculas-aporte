...
export const getFavorites = asyncHandler(async (req, res) => {
  const favorites = await query("SELECT * FROM favorites WHERE user_id = ? ORDER BY created_at DESC", [req.user.id]);
  res.json({ favorites });
});
...
export const removeFavorite = asyncHandler(async (req, res) => {
  await query("DELETE FROM favorites WHERE user_id = ? AND tmdb_id = ?", [req.user.id, req.params.tmdb_id]);
